...
export const Sidebar = ({ onLogoutClick }: Props) => {
		...
		<>
			...
			>
				<div className="h-16 shrink-0 flex items-center justify-between px-4 border-b border-gray-100 dark:border-gray-800">
					<div
						className={`flex items-center gap-3 overflow-hidden transition-all duration-300 ${!isOpen && "md:w-0"}`}
					>
						<Link
							to="/admin/dashboard"
				...
				<div className="flex-1 overflow-y-auto overflow-x-hidden main-scrollbar py-4 px-3 flex flex-col gap-1.5">
					{sidebarItems.map((item) => {
						const showItem =
							!item.restricted || isSuperAdmin(user?.rol || "");
						...
						if (item.subItems) {
							return (
								<SidebarSubmenu
									...
									isCompact={!isOpen}
									onClick={
										!isOpen ? toggleSidebar : undefined
									}
