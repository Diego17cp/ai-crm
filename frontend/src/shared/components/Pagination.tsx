...
const PaginationButton = ({
	...
	<motion.button
		...
		className={`
            ...
            ${
				active
					? "bg-pink-600 text-white shadow-md shadow-pink-500/20 cursor-auto"
					: disabled
...
export const Pagination = ({
	...
	return (
		...
		>
			...
			<div
				className={`flex items-center gap-1.5 ${isLoading ? "opacity-60 pointer-events-none" : "transition-opacity"}`}
			>
				<PaginationButton
				...
				<AnimatePresence>
					{isLoading && (
						<motion.div
							...
							className="ml-2"
						>
							<FiLoader className="w-4 h-4 text-pink-500 animate-spin" />
						</motion.div>
					)}
				</AnimatePresence>
			</div>
