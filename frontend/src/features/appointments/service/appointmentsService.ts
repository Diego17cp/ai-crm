...
export const appointmentsService = {
	getAppointments: async (filters: FiltersState) => {
		...
		if (filters.estado_cita)
			params.append("estado_cita", filters.estado_cita);
		if (filters.fecha_inicio)
			params.append("fecha_inicio", filters.fecha_inicio);
		if (filters.fecha_fin) params.append("fecha_fin", filters.fecha_fin);
		if (filters.puntuacion)
			params.append("puntuacion", filters.puntuacion.toString());
		if (filters.id_proyecto)
			params.append("id_proyecto", filters.id_proyecto.toString());
		if (filters.id_usuario_responsable)
			params.append(
