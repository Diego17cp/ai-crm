...
export const DeleteAppointmentModal = ({ isOpen, onClose, cita }: Props) => {
    const [error, setError] = useState<string | null>(null);

    const { useDeleteAppointmentMutation } = useAppointments();
    const deleteMutation = useDeleteAppointmentMutation(cita?.id || 0);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const message = (err as ApiError)?.response?.data?.message || "Error al eliminar la cita.";
            setError(message);
        }
    };
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
                                ...
                                <div className="text-sm text-gray-700 dark:text-gray-300 ml-1 leading-relaxed">
                                    ¿Estás seguro de que deseas eliminar permanentemente la cita programada con <strong className="text-gray-900 dark:text-white capitalize">{clienteNombre}</strong>?
                                    <span className="block mt-2 text-red-600 dark:text-red-400 font-medium">
                                        Esta acción eliminará el registro de la cita del sistema.
                                    </span>
                                </div>
