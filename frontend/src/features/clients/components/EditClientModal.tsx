...
export const EditClientModal = ({ isOpen, onClose, client }: Props) => {
	...
	useEffect(() => {
		if (isOpen && client) {
			setNumeroDoc(client.numero || "");
			setNombres(client.nombres || "");
			setApellidos(client.apellidos || "");
			setEmail(client.email || "");
			setDireccion(client.direccion || "");
			setOcupacion(client.ocupacion || "");
			setFechaNacimiento(
				client.fecha_nacimiento
					? client.fecha_nacimiento.split("T")[0]
					: "",
			);

			setEsPeruano(
				client.es_peruano !== null ? String(client.es_peruano) : "true",
			);
			setSexo(client.sexo || "");
			setEstadoCivil(client.estado_civil || "");
			setSolvencia(client.solvencia || "");
			setActitud(client.actitud || "");
			setIdUbigeo(client.id_ubigeo || "");

			if (client.telefonos) {
				setPhones(
					client.telefonos.map((t) => ({
	...
	const updatePayload = useMemo((): UpdateLeadPayload => {
		...
		return {
			id_tipo_doc_identidad: client?.id_tipo_doc_identidad || 1,
