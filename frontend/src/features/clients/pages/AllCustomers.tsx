...
export const AllCustomers = () => {
	...
	const { activeModal, selectedClient, openModal, closeModal } =
		useClientModals();
	return (
		<div className="flex flex-col gap-6 w-full max-w-7xl mx-auto pb-10">
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				...
				>
					<h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
						<FiUsers className="text-pink-600 dark:text-pink-500" />{" "}
				...
				<div className="flex items-center gap-3">
					...
					<button
						...
						className="p-3 flex items-center gap-2 cursor-pointer shrink-0 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-pink-600 dark:hover:text-pink-500 transition-all disabled:opacity-50"
					>
						<FiRefreshCw
				...
				<div className="flex flex-col lg:flex-row gap-4 items-center justify-between w-full">
					<div className="relative w-full lg:w-96 shrink-0 z-30">
						...
						<input
							...
							className="w-full pl-10 pr-10 py-2.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-colors"
						/>
						<AnimatePresence>
							{searchTerm && (
								<motion.button
						...
						<AnimatePresence mode="popLayout">
							{clients.map((client) => (
								<ClientCard
									...
									client={client}
									onEdit={() =>
										openModal("edit_client", client)
									}
									onDelete={() =>
										toast.warning(
											"Un cliente no puede ser eliminado.",
										)
									}
								/>
							))}
			...
			<CreateClientModal
				isOpen={activeModal === "create_client"}
				onClose={closeModal}
			/>
			<EditClientModal
