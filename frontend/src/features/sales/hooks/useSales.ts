...
export const useSales = () => {
	...
	const useCreateSaleMutation = (payload: CreateSalePayload) =>
		useMutation({
	...
	const usePayQuotaMutation = (id: number, metodoPago: MetodoPago) =>
		useMutation({
			...
			onError: (error: ApiError) => {
				const message =
					error.response?.data?.message || "Error al pagar la cuota";
				toast.error(message);
			},
		});
