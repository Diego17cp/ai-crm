			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				...
				>
					<h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
						<FiDollarSign className="text-pink-600 dark:text-pink-500" />
						Control de Cobranzas
					</h1>
					<p className="text-sm text-gray-500 dark:text-gray-400">
						Monitorea y gestiona las cuotas pendientes de todos los
						contratos.
					</p>
				</motion.div>
				...
				<div className="flex items-center gap-3">
					<button
						...
						className="p-3 cursor-pointer flex gap-2 items-center shrink-0 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-pink-600 dark:hover:text-pink-500 transition-all disabled:opacity-50"
					>
						<FiRefreshCw
				...
				{isError ? (
					...
					>
						...
						<p className="text-gray-500 mt-1 max-w-md">
							...
							{currentTab === "vencidas"
								? "vencidas"
								: "pendientes por mostrar en este filtro"}
							. ¡Buen trabajo!
						</p>
					</motion.div>
				) : (
						...
						<AnimatePresence mode="popLayout">
							{cobros.map((cobro) => (
								<CollectionListItem
									...
									onRemindClick={(cuotaId) =>
										notifyReminderMutation.mutate(cuotaId)
									}
									isReminding={
										notifyReminderMutation.isPending &&
										notifyReminderMutation.variables ===
											cobro.id
									}
								/>
							))}
