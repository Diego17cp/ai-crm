...
export const QuotaNotificationsModal = ({
	isOpen,
	onClose,
	notificaciones,
	cuotaNro,
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-gray-500 dark:text-gray-400">
											Recordatorios de Cuota N° {cuotaNro}
										</p>
									</div>
								</div>
								<button
							...
							<div className="p-6 overflow-y-auto main-scrollbar flex flex-col gap-4">
								{notificaciones.length === 0 ? (
									...
									notificaciones.map((notif) => (
										<div
											key={notif.id}
											className="p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 relative"
										>
											<span
												className={`absolute top-4 right-4 px-2 py-0.5 text-xs font-semibold rounded-md border ${getUrgencyLevelColor(notif.nivel_urgencia)}`}
											>
												{getUrgencyLevelLabel(
