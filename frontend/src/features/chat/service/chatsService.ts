...
export const chatsService = {
	...
	getChatById: async (chatId: string) => {
		const response = await apiClient.get<ChatDetailsResponse>(
			`/chats/${chatId}`,
		);
		return response.data;
	},
	...
	updateStatusChat: async (chatId: string, newStatus: ChatStatus) => {
		const response = await apiClient.patch(`/chats/${chatId}/status`, {
			newStatus,
		});
		return response.data;
	},
	getEventsByChatId: async (chatId: string) => {
		const response = await apiClient.get<{ data: EventChat[] }>(
			`/chats/${chatId}/events`,
		);
		return response.data;
	},
	getAssignationsByChatId: async (chatId: string) => {
		const response = await apiClient.get<{ data: Assignment[] }>(
			`/chats/${chatId}/assignments`,
		);
		return response.data;
	},
};
