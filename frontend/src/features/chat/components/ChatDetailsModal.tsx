...
import { ChatDetailTabs, type ChatDetailTab } from "./ChatDetailTabs";
...
export const ChatDetailsModal = ({
	...
	const {
		useChatByIdQuery,
		useEventsByChatIdQuery,
		useAssignationsByChatIdQuery,
	} = useChats();
	const [activeTab, setActiveTab] = useState<ChatDetailTab>("mensajes");

	const {
		data: response,
		isLoading,
		isError,
	} = useChatByIdQuery(chatId || "");
	const chatDetails = response?.data;

	const eventsQuery = useEventsByChatIdQuery(
		activeTab === "eventos" ? chatId || "" : "",
	);
	const assignmentsQuery = useAssignationsByChatIdQuery(
		activeTab === "asignaciones" ? chatId || "" : "",
	);

	const messagesEndRef = useRef<HTMLDivElement>(null);

	useEffect(() => {
		if (chatDetails?.mensajes?.length && activeTab === "mensajes") {
			messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
		}
	}, [chatDetails?.mensajes, activeTab]);
	...
	useEffect(() => {
		if (isOpen) setActiveTab("mensajes");
	}, [chatId, isOpen]);
				...
				<>
					...
					<div className="fixed inset-0 z-115 flex items-center justify-center p-4 sm:p-6 pointer-events-none">
						<motion.div
							...
							className="bg-white dark:bg-gray-900 w-full max-w-4xl max-h-[85vh] flex flex-col rounded-3xl shadow-2xl pointer-events-auto border border-gray-100 dark:border-gray-800 overflow-hidden"
						>
										...
										<div className="flex flex-col">
											<h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white leading-tight flex items-center gap-2">
												{chatDetails.cliente
													...
													: "Cliente Anónimo"}
												<span
													className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${getEstadoChatConfig(chatDetails.estado).colorClass}`}
												>
													{
														getEstadoChatConfig(
															chatDetails.estado,
											...
											<div className="flex flex-col gap-6 w-full">
												{[...Array(4)].map((_, i) => {
													...
													return (
														<div
															key={i}
															className={`flex max-w-[80%] ${isRight ? "self-end justify-end" : "self-start"} w-full gap-3`}
														>
															{!isRight && (
																<div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-800 animate-pulse shrink-0" />
															)}
															<div
																className={`h-16 w-3/4 sm:w-1/2 bg-gray-200 dark:bg-gray-800 animate-pulse ${isRight ? "rounded-l-2xl rounded-tr-2xl" : "rounded-r-2xl rounded-tl-2xl"}`}
															/>
														</div>
													);
												})}
															...
															>
																{!isClient && (
																	<div className="shrink-0 mt-3 sm:mt-4">
																		{isBot ? (
																			<div className="size-8 sm:size-9 rounded-full bg-pink-600 flex items-center justify-center text-white shadow-sm">
																				<VscRobot
																					size={
																						20
																					}
																...
																<div
																	className={`flex flex-col w-full ${isClient ? "items-end" : "items-start"}`}
																>
																	{!isClient && (
																	...
																	>
																		{isBot ? (
																			<div className="prose prose-sm dark:prose-invert max-w-none prose-p:leading-snug prose-a:text-pink-500 prose-ul:pl-4">
																				<ReactMarkdown>
																					{
																						contenidoMensaje
																					}
																		...
																		{msg.adjunto &&
																			msg
																				...
																				.length >
																				0 && (
																				<div className="flex flex-col items-center gap-2 px-4 pb-4 pt-3 border-t border-gray-200/70 dark:border-gray-700/70">
																					{msg.adjunto.map(
																						(
																							att,
																							i,
																	...
																	<span
																		className={`text-[10px] text-gray-400 dark:text-gray-500 mt-1 ${isClient ? "mr-1" : "ml-1"}`}
																	>
																		{formatChatDate(
																			msg.created_at,
																		)}
