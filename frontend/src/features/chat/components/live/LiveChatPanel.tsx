...
export const LiveChatPanel = ({
	...
	const nombreCliente =
		chatData?.cliente && chatData?.cliente.nombres
		...
		<div className="flex-1 flex flex-col h-full bg-[#f8fafc] dark:bg-[#0b1120]">
			...
			<div className="flex-1 overflow-y-auto p-6 space-y-4 main-scrollbar">
				{chatData?.mensajes.map((msg) => {
					...
					const isAsesor =
						msg.remitente === "HUMANO" && !!msg.usuario;
					...
					return (
						...
						>
							...
							>
								...
								<span
									className={`text-[10px] mt-1 block opacity-70 ${!isClient ? "text-right" : "text-left"}`}
								>
