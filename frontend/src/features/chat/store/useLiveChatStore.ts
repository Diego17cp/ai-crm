...
export const useLiveChatStore = create<LiveChatState>()(
	devtools(
		(set) => ({
			...
			addChatToQueue: (chat) =>
				set((state) => {
					if (state.queueQueue.some((c) => c.id === chat.id))
						return state;
					return {
						queueQueue: [chat, ...state.queueQueue],
					};
				}),
			removeChatFromQueue: (chatId) =>
				set((state) => ({
					queueQueue: state.queueQueue.filter(
						(chat) => chat.id !== chatId,
					),
				})),
			moveChatToActive: (chatId) =>
				set((state) => {
					const chatToMove = state.queueQueue.find(
						(chat) => chat.id === chatId,
					);
					if (!chatToMove) return state;
					const isAlreadyActive = state.activeChats.some(
						(chat) => chat.id === chatId,
					);
					return {
						queueQueue: state.queueQueue.filter(
						...
						activeChats: isAlreadyActive
							? state.activeChats
							: [chatToMove, ...state.activeChats],
					};
				}),
			updateChatLastMessage: (chatId, message) =>
				set((state) => ({
					...
					queueQueue: state.queueQueue.map((chat) =>
						chat.id === chatId
							? { ...chat, lastMessage: message }
							: chat,
					),
