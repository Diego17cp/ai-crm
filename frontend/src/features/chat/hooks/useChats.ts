...
export const useChats = () => {
	...
	const useChatByIdQuery = (chatId: string) =>
		useQuery({
			queryKey: ["chat", chatId],
			queryFn: () => chatsService.getChatById(chatId),
			enabled: Boolean(chatId),
		});
	const useAssignationsByChatIdQuery = (chatId: string) =>
		useQuery({
			queryKey: ["assignations", chatId],
			queryFn: () => chatsService.getAssignationsByChatId(chatId),
			enabled: Boolean(chatId),
		});
	const useEventsByChatIdQuery = (chatId: string) =>
		useQuery({
			queryKey: ["events", chatId],
			queryFn: () => chatsService.getEventsByChatId(chatId),
			enabled: Boolean(chatId),
		});
