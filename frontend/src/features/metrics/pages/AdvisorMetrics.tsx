...
export const AdvisorMetrics = () => {
	...
	const { data, isLoading, isError, refetch, isRefetching } =
		useAdvisorMetrics(idUsuario, desde, hasta);
	...
	return (
		<div className="flex flex-col gap-6 w-full max-w-7xl mx-auto pb-10">
			{seingOther && (
				<Link
					to="/admin/dashboard"
					className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-pink-600 dark:hover:text-pink-400 mb-2 transition-colors"
				>
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				...
				>
					<h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
						<GrPieChart className="text-pink-600 dark:text-pink-500" />{" "}
						{seingOther
							? `Desempeño de ${nameFromNav ?? "asesor"}`
							: "Mi Desempeño"}
					</h1>
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
				<MetricCard
					...
					value={resumen.clientes_atendidos}
					accent="blue"
				/>
				<MetricCard
					icon={FiCalendar}
					label="Citas agendadas"
					value={resumen.citas_agendadas}
					accent="pink"
				/>
				<MetricCard
					icon={FiFileText}
					label="Cotizaciones generadas"
					value={resumen.cotizaciones_generadas}
					accent="purple"
				/>
				<MetricCard
					icon={FiDollarSign}
					label="Ventas cerradas"
					value={resumen.ventas_cerradas}
					accent="amber"
				/>
				<MetricCard
					icon={FiDollarSign}
					label="Monto vendido"
					value={formatCurrency(resumen.monto_vendido)}
					accent="pink"
				/>
				<MetricCard
					icon={FiUsers}
					label="Tasa de cierre"
					value={formatPercent(resumen.tasa_cierre)}
					accent="blue"
				/>
				<MetricCard
					icon={FiClock}
					label="Tiempo resp. promedio"
					value={formatDuration(resumen.tiempo_respuesta_prom_seg)}
					accent="purple"
				/>
				<MetricCard
					icon={FiStar}
					label="Calificación promedio"
					value={formatScore(resumen.puntuacion_atencion_prom)}
					accent="amber"
				/>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<MetricsTrendChart
					data={serie_diaria}
					dataKey="clientes_atendidos"
					title="Clientes atendidos por día"
					color="#2563eb"
				/>
				<MetricsTrendChart
					data={serie_diaria}
					dataKey="ventas_cerradas"
					title="Ventas cerradas por día"
					color="#d97706"
				/>
				<MetricsTrendChart
					data={serie_diaria}
					dataKey="tiempo_respuesta_prom_seg"
					title="Tiempo de respuesta (seg)"
					color="#7c3aed"
				/>
				<MetricsTrendChart
					data={serie_diaria}
					dataKey="puntuacion_atencion_prom"
