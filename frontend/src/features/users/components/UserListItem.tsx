...
export const UserListItem = ({ user, onEdit, onDelete }: Props) => {
	return (
		...
		>
			<div
				className={`absolute left-0 top-0 bottom-0 w-1 ${user.estado === "ACTIVO" ? "bg-pink-500" : "bg-gray-300 dark:bg-gray-700"}`}
			></div>
			...
			<div className="flex flex-row md:flex-col items-center md:items-start justify-between md:justify-center w-full md:w-1/6 shrink-0 gap-2">
				...
				<span
					className={`px-2 py-0.5 text-sm font-bold rounded uppercase tracking-wide border ${
						user.estado === "ACTIVO"
