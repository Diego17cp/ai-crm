...
export const useLots = () => {
	...
	const [filters, setFilters] = useState({
		...
		precio_total_min: undefined as number | undefined,
		precio_m2_min: undefined as number | undefined,
	...
	const clearAllFilters = () => {
		...
		setFilters({
			...
			estado: undefined,
			precio_total_min: undefined,
			precio_m2_min: undefined,
			area: undefined,
		});
		goToPage(1);
	};
	...
	const hasActiveFilters = Boolean(
		...
		filters.estado ||
		filters.area ||
		filters.precio_total_min ||
		filters.precio_m2_min
	);
	...
	const query = useQuery({
		queryKey: [
			...
			queryPayload.estado,
			queryPayload.precio_total_min,
			queryPayload.precio_m2_min,
			queryPayload.area,
		],
	...
	const etapasQuery = useQuery({
		...
		queryFn: async () => {
			...
			const response = await apiClient.get(
				`/projects/etapas/${filters.id_proyecto}`,
			);
			return response.data.data;
		},
	});
	const manzanasQuery = useQuery({
		...
		queryFn: async () => {
			...
			const response = await apiClient.get(
				`/projects/etapas/${filters.id_etapa}/manzanas`,
			);
			return response.data.data;
		},
	});

	const useCreateLoteMutation = (
		data: Omit<Lote, "id" | "created_at" | "imagenes" | "manzana">,
		...
		useMutation({
			...
			onError: (error: ApiError) => {
				const message =
					error?.response?.data?.message || "Error al crear lote";
				toast.error(message);
			},
		});
	const useUpdateLoteMutation = (
		id: number,
		data: Partial<Omit<Lote, "id" | "created_at" | "imagenes" | "manzana">>,
		imagenesUpdate: ImageUpdateInput,
		archivosNuevos: File[],
	) =>
		useMutation({
			mutationFn: () =>
				lotsService.update(id, data, imagenesUpdate, archivosNuevos),
			onSuccess: () => {
				queryClient.invalidateQueries({ queryKey: ["lotes"] });
	...
	const useDeleteLoteMutation = (id: number) =>
		useMutation({
			...
			onError: (error: ApiError) => {
				const message =
					error?.response?.data?.message || "Error al eliminar lote";
				toast.error(message);
			},
		});
