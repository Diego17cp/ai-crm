...
export const LotCard = ({ lote, onEdit, onDelete }: Props) => {
	...
	const formattedPrice = new Intl.NumberFormat("es-PE", {
		style: "currency",
		currency: "PEN",
	}).format(Number(lote.precio_total));
	...
	return (
		...
		>
			<div className="relative h-60 bg-gray-100 dark:bg-gray-800 w-full overflow-hidden flex items-center justify-center">
				...
				<div
					className={`absolute top-3 left-3 px-3 py-1 rounded-lg text-xs font-bold border backdrop-blur-md shadow-sm ${statusStyle.color}`}
				>
					{statusStyle.label}
				</div>
			...
			<div className="p-5 flex flex-col flex-1">
				...
				<div className="flex-1 space-y-3 mt-1">
					<div className="flex items-start gap-2.5 text-sm text-gray-600 dark:text-gray-400">
						...
						<div className="flex flex-col leading-tight">
							...
							<span className="text-xs">
								{lote.manzana?.etapa?.nombre || "Etapa"} • Mz.{" "}
								{lote.manzana?.codigo || "---"}
							</span>
						</div>
					</div>
					<div className="flex items-center gap-2.5 text-sm text-gray-600 dark:text-gray-400">
						...
						<div className="flex flex-col leading-tight">
							<span className="font-medium text-gray-800 dark:text-gray-200">
								{lote.ubicacion_referencial ||
									"Sin ubicación referencial"}
							</span>
						</div>
					</div>
					<div className="flex items-center gap-2.5 text-sm text-gray-600 dark:text-gray-400">
