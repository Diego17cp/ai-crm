...
export const ProjectDetailsModal = ({
	...
	const totalManzanas = project.etapas.reduce(
		(acc, etapa) => acc + etapa.manzanas.length,
		0,
	);
							...
							<div className="flex justify-between items-center px-6 py-5 border-b border-gray-100 dark:border-gray-800 shrink-0 bg-gray-50/50 dark:bg-gray-800/20">
								<div className="flex flex-col gap-1">
									<div className="flex items-center gap-3">
										...
										<span
											className={`px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider rounded-lg ${getBadgeStyle(project.estado)}`}
										>
											{project.estado}
										</span>
									</div>
															...
															{etapa.manzanas.map(
																(mz) => (
																	<div
																		...
																		className="group/mz relative flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs rounded-lg border border-gray-200/60 dark:border-gray-700 font-medium pb-1.5 hover:border-blue-400 dark:hover:border-blue-500 transition-colors"
																	>
																		<FiBox
																			size={
																				11
																			}
																			className="text-gray-400 group-hover/mz:text-blue-500"
																		/>
																		Mz{" "}
																		{
																			mz.codigo
