...
export const CreateEtapaModal = ({ isOpen, onClose, project }: Props) => {
	...
	const createEtapaMutation = useCreateEtapaMutation(
		project?.id || 0,
		nombre,
	);
	...
	const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
		...
		if (!trimmedName) {
			...
			return setError("El nombre de la etapa es obligatorio.");
		}
		...
		try {
			createEtapaMutation.mutate(undefined, {
				onSuccess: () => {
					onClose();
				},
				onError: (err: unknown) => {
