...
export const CreateManzanaModal = ({
	isOpen,
	onClose,
	project,
	etapa,
	...
	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		setError(null);
		const limpiarCodigos = codigos
			.map((c) => c.trim())
			.filter((c) => c !== "");
		if (limpiarCodigos.length === 0) {
			toast.error("Debes ingresar al menos un código de manzana.");
			setError("Debes ingresar al menos un código de manzana.");
			return;
		}
		try {
			createManzanaMutation.mutate(undefined, {
				onSuccess: () => {
					onClose();
				},
				onError: (err: unknown) => {
			...
			console.error("Error creating manzana:", err);
		}
	};
