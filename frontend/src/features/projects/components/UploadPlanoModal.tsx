...
export const UploadPlanoModal = ({ isOpen, onClose, project }: Props) => {
	...
	const uploadMutation = useUploadPlanoMutation(
		project?.id || 0,
		file as File,
	);
	...
	const pdfUrl = project.plano_url
		? `${baseUrl}/uploads/planos/${project.plano_url}`
		: "";
							...
							>
									...
									{project.plano_url && (
										<div className="flex flex-col gap-3 p-4 bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30 rounded-2xl">
											...
											>
												...
												<FiDownload
													size={18}
													className="text-gray-400 group-hover:text-orange-500 mr-1 shrink-0"
												/>
											</a>
										</div>
									)}
													...
													classes={{
														container: ` ${
															isDragging
																...
																: "border-gray-300 dark:bg-gray-800! dark:border-gray-700! hover:border-pink-500! hover:bg-gray-50 dark:hover:bg-gray-800/50!"
														}`,
													}}
												/>
											</div>
										) : (
											<div className="flex items-center justify-between p-4 border border-pink-200 dark:border-pink-900/50 bg-pink-50 dark:bg-pink-900/10 rounded-2xl">
												<div className="flex items-center gap-3 overflow-hidden">
													<div className="p-2.5 bg-white dark:bg-gray-800 text-pink-600 dark:text-pink-400 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 shrink-0">
														<FiFileText size={20} />
													</div>
								...
								<div className="p-5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 flex justify-end gap-3 shrink-0">
									...
									<button
										...
										className="flex items-center cursor-pointer gap-2 bg-pink-600 hover:bg-pink-700 active:bg-pink-800 text-white px-6 py-2.5 rounded-xl font-medium shadow-sm shadow-pink-500/30 transition-all focus:ring-2 focus:ring-pink-500 focus:outline-none disabled:opacity-50 disabled:shadow-none"
									>
										{isSubmitting ? (
											<>
												<BiLoaderAlt
