...
export const ToggleEtapaStatusModal = ({ isOpen, onClose, project, etapa }: Props) => {
    const [error, setError] = useState<string | null>(null);

    const { useToggleEtapaStatusMutation } = useEtapas();
    const toggleEtapaStatusMutation = useToggleEtapaStatusMutation(etapa ? etapa.id : 0);
    const isSubmitting = toggleEtapaStatusMutation.isPending;

    const isActiva = etapa?.estado === "ACTIVO";
    ...
    const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        setError(null);
        try {
            toggleEtapaStatusMutation.mutate();
            onClose();
        } catch (err: unknown) {
            const message = (err as ApiError)?.response?.data?.message || "Error al actualizar estado de etapa";
            setError(message);
        }
    };
                        ...
                        >
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        <h2 className="text-lg font-bold text-gray-900 dark:text-white leading-tight">
                                            {isActiva ? "Desactivar Etapa" : "Activar Etapa"}
                                        </h2>
                            ...
                            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
                                ...
                                <div className="text-sm text-gray-700 dark:text-gray-300 ml-1 leading-relaxed">
                                    {isActiva 
                                        ? <>¿Estás seguro de que deseas desactivar la etapa <strong className="text-gray-900 dark:text-white">{etapa.nombre}</strong>? Esta acción la ocultará en ciertas vistas dependientes y no se podrán crear lotes dentro de la misma.</>
                                        : <>¿Deseas volver a activar la etapa <strong className="text-gray-900 dark:text-white">{etapa.nombre}</strong>? Volverá a estar disponible para su operativa normal.</>
                                    }
                                </div>
