...
export const ToggleEtapaStatusModal = ({
	isOpen,
	onClose,
	project,
	etapa,
	...
	const toggleEtapaStatusMutation = useToggleEtapaStatusMutation(
		etapa ? etapa.id : 0,
	);
	...
	const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
		...
		try {
			...
			const message =
				...
				"Error al actualizar estado de etapa";
			setError(message);
		}
	};
						...
						>
							<div className="flex justify-between items-center px-6 py-5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/20">
								<div className="flex items-center gap-3">
									<div
										className={`p-2 rounded-xl ${isActiva ? "bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400" : "bg-pink-100 dark:bg-pink-900/40 text-pink-600 dark:text-pink-400"}`}
									>
										{isActiva ? (
								...
								<div className="text-sm text-gray-700 dark:text-gray-300 ml-1 leading-relaxed">
									{isActiva ? (
										<>
											...
											? Esta acción la ocultará en ciertas
											vistas dependientes y no se podrán
											crear lotes dentro de la misma.
										</>
									) : (
										<>
											¿Deseas volver a activar la etapa{" "}
											<strong className="text-gray-900 dark:text-white">
												{etapa.nombre}
											</strong>
											? Volverá a estar disponible para su
											operativa normal.
										</>
									)}
								</div>
									...
									<button
										...
										className={`flex-1 cursor-pointer py-3 px-4 text-white font-semibold rounded-xl shadow-md transition-all flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed ${
											isActiva
												...
												: "bg-pink-600 hover:bg-pink-700 active:bg-pink-800 shadow-pink-500/20"
										}`}
									>
										{isSubmitting ? (
											<>
