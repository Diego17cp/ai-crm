...
export class SocketChatController {
	...
	registerListeners(socket: Socket) {
		...
		socket.on(
			"client:TAKE_CHAT",
			async (payload: { chatId: string; asesorId: string }) => {
				try {
					await this.chatUseCases.assignChatFromQueue(
						payload.chatId,
						payload.asesorId,
					);
					this.io.emit("server:CHAT_ASSIGNED", {
		...
		socket.on(
			...
			async (payload: {
				...
				try {
					await this.chatUseCases.saveLiveChatMessage(
						payload.chatId,
						payload.content,
						payload.senderRole,
					);
					if (payload.senderRole === "ASESOR") {
						const chat = await this.chatUseCases.getChatById(
							payload.chatId,
						);
		...
		socket.on(
			"client:FORCE_TAKE_CHAT",
			async (payload: { chatId: string; asesorId: string }) => {
				try {
					await this.chatUseCases.forceAssignChat(
						payload.chatId,
						payload.asesorId,
					);
					this.io.emit("server:CHAT_ASSIGNED", {
