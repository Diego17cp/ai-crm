...
export class ProyectosController {
	...
	getAll = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const result = await this.proyectosUseCases.getAllProyectos({
				q,
				page,
				limit,
			});
	...
	getAllSimple = async (req: Request, res: Response, next: NextFunction) => {
		try {
			const proyectos =
				await this.proyectosUseCases.getAllProyectosSimple();
	...
	update = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const data = req.body;
			const proyecto = await this.proyectosUseCases.updateProyecto(
				id,
				data,
			);
	...
	delete = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			res.status(200).json({
				success: true,
				data: proyecto,
				message: "Proyecto eliminado (lógico)",
			});
		} catch (error) {
			next(error);
		}
	...
	createEtapa = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const etapa = await this.proyectosUseCases.createEtapa({
				...req.body,
				id_proyecto,
			});
	...
	deleteEtapa = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			res.status(200).json({
				success: true,
				data: etapa,
				message: "Etapa eliminada (lógico)",
			});
		} catch (error) {
			next(error);
		}
	};
	getAllEtapas = async (req: Request, res: Response, next: NextFunction) => {
		try {
			const etapas = await this.proyectosUseCases.findAllEtapas();
			res.status(200).json({ success: true, data: etapas });
		} catch (error) {
			next(error);
		}
	};
	getEtapasByProyectoId = async (
		req: Request,
		res: Response,
		next: NextFunction,
	) => {
		try {
			const id_proyecto = Number(req.params.id_proyecto);
			const etapas =
				await this.proyectosUseCases.findEtapasByProyectoId(
					id_proyecto,
				);
			res.status(200).json({ success: true, data: etapas });
		} catch (error) {
	...
	getAllManzanas = async (
		_: Request,
		res: Response,
		next: NextFunction,
	) => {
		try {
			const manzanas = await this.proyectosUseCases.findAllManzanas();
			res.status(200).json({ success: true, data: manzanas });
		} catch (error) {
			next(error);
		}
	};
	getManzanasByEtapaId = async (
		req: Request,
		res: Response,
		next: NextFunction,
	) => {
		try {
			...
			const manzanas =
				await this.proyectosUseCases.findManzanasByEtapaId(id_etapa);
			res.status(200).json({ success: true, data: manzanas });
		} catch (error) {
			next(error);
		}
	...
	createManzana = async (req: Request, res: Response, next: NextFunction) => {
		try {
			const id_etapa = Number(req.params.id_etapa);
			const manzana = await this.proyectosUseCases.createManzana({
				...req.body,
				id_etapa,
			});
	...
	createManzanasBatch = async (
		...
		try {
			...
			const count = await this.proyectosUseCases.createManzanasBatch({
				id_etapa,
				codigos,
			});
			res.status(201).json({
				success: true,
				count,
				message: `${count} manzanas creadas`,
			});
		} catch (error) {
			next(error);
		}
	...
	updateManzana = async (req: Request, res: Response, next: NextFunction) => {
		try {
			const id = Number(req.params.id);
			const data = req.body;
			const manzana = await this.proyectosUseCases.updateManzana(
				id,
				data,
			);
	...
	deleteManzana = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			res.status(200).json({
				success: true,
				data: manzana,
				message: "Manzana eliminada (lógico)",
			});
		} catch (error) {
			next(error);
		}
