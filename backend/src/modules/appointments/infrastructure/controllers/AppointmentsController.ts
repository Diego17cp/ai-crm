...
const formatCitaResponse = (cita: any) => {
	...
	if (formatted.hora_cita && formatted.hora_cita instanceof Date) {
		const hours = formatted.hora_cita
			.getUTCHours()
			.toString()
			.padStart(2, "0");
		const minutes = formatted.hora_cita
			.getUTCMinutes()
			.toString()
			.padStart(2, "0");
...
export class AppointmentsController {
	...
	getAll = async (req: AuthRequest, res: Response, next: NextFunction) => {
		try {
			...
			const id_usuario_responsable = isAdmin ? undefined : req.user!.id;
			const query: GetAppointmentsQueryDTO = {
				...
				puntuacion: req.query.puntuacion
					? parseInt(req.query.puntuacion as string)
					: undefined,
	...
	update = async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const updatedAppointment = await this.useCases.updateAppointment(
				id,
				data,
			);
			res.status(200).json({
