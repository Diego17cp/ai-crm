...
const parseHoraCita = (horaStr: string | Date): Date => {
	...
	const baseDate = horaStr.includes("T")
		? horaStr
		: `1970-01-01T${horaStr}:00.000Z`;
...
export class AppointmentsUseCases {
	...
	async createAppointment(data: CreateAppointmentDTO) {
		if (!data.id_cliente && !data.id_lead && !data.nuevo_lead)
			throw new AppError(
				"Debe especificar un cliente o lead existente o los datos para crear uno nuevo",
				400,
			);
		const fecha = new Date(data.fecha_cita);
		const hora = parseHoraCita(data.hora_cita);
		if (isNaN(fecha.getTime()) || isNaN(hora.getTime()))
			throw new AppError("Fecha u hora de cita inválida", 400);
		...
		if (conflictos.length > 0)
			throw new AppError(
				"Ya existe una cita en esa fecha y hora para este lote o este asesor",
				409,
			);
	...
	async updateAppointment(id: number, data: UpdateAppointmentDTO) {
		...
		if (
			data.puntuacion_cliente !== undefined &&
			data.puntuacion_cliente !== null
		) {
			if (data.puntuacion_cliente < 1 || data.puntuacion_cliente > 5)
				throw new AppError(
					"La puntuación debe ser un valor entre 1 y 5 estrellas",
					400,
				);
		}
		...
		if (recalarConflicto || data.id_lote || data.id_usuario_responsable) {
			const checkLote =
				data.id_lote === null
					? undefined
					: (data.id_lote ?? existing.id_lote);
			const checkAsesor =
				data.id_usuario_responsable ?? existing.id_usuario_responsable;
			const conflictos = await this.repo.findConflictingAppointments(
				fecha,
				hora,
				checkLote,
			...
			if (conflictos.length > 0)
				throw new AppError(
					"La modificación genera un solapamiento con otra cita en la misma fecha y hora",
					409,
				);
		}
		...
		return this.repo.update(
			id,
			data,
			data.fecha_cita ? fecha : undefined,
			data.hora_cita ? hora : undefined,
		);
	}
