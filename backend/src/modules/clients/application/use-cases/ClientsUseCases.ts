...
export class ClientsUseCases {
	...
	async createClient(data: CreateClientDTO) {
		...
		const existingClient = await this.repo.findByDocument(
	...
	async updateClient(id: number, data: UpdateClientDTO) {
		...
		if (data.id_tipo_doc_identidad || data.numero) {
			...
			const clientDocDuplicate = await this.repo.findByDocument(
				checkDocType,
				checkNum,
			);

			if (clientDocDuplicate && clientDocDuplicate.id !== id)
				throw new AppError(
		...
		if (data.telefonos) {
			...
			if (remove && Array.isArray(remove)) {
				for (const telId of remove) {
					const belongsToClient = existing.persona.telefonos.some(
						(t) => t.id === telId,
					);
					if (!belongsToClient)
						throw new AppError(
							`El teléfono con ID ${telId} no pertenece al Cliente`,
							400,
						);
				}
			}
			if (update && Array.isArray(update)) {
				for (const tel of update) {
					...
					if (!belongsToClient)
						throw new AppError(
							`El teléfono con ID ${tel.id} no pertenece al Cliente`,
							400,
						);
	...
	async deleteClient(id: number) {
		...
		if (hasSales)
			throw new AppError(
				"No se puede eliminar el cliente porque tiene ventas o deudas asociadas.",
				409,
			);
